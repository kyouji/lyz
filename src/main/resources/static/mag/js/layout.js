...
$(function () {
    ...
    $(".ltable1 tr:nth-child(odd)").addClass("odd_bg"); //隔行变色
    $("#floatHead").smartFloat_floatHead();
    $(".location").smartFloat_location();
    ...
    $(".rule-single-checked").ruleSingleChecked();
});
//全选取消按钮函数
...
//执行客服接单函数
function ExeKefuPostBack(objId, objmsg) {

    var msg = "";
    if (objmsg == "") {
        msg = "您确定要对当前标的接单，进行服务吗？";
    }
    else {
        msg = objmsg;
    }
...
//执行确认函数
function ExeYesPostBack(objId, objmsg) {
...
//执行驳回函数
function ExeNoPostBack(objId, objmsg) {
    var msg = "";
    if (objmsg == "") {
        msg = "您确定要对当前标的进行驳回吗？";
    }
...
//执行取消函数
function ExeCancelPostBack(objId, objmsg) {
...
//智能浮动层函数
$.fn.smartFloat_floatHead = function () {
    var position = function (element) {
        var top = element.position().top;
        var pos = element.css("position");
        $(window).scroll(function () {
...
//智能浮动层函数
$.fn.smartFloat_location = function () {
    var position = function (element) {
        var top = element.position().top;
        var pos = element.css("position");
        $(window).scroll(function () {
    ...
    var singleCheckbox = function (parentObj) {
        ...
        parentObj.find(":checkbox").each(function () {
            ...
            $(newObj).click(function () {
                ...
                parentObj.find(':checkbox').attr("checked", false);
                parentObj.find(':checkbox').eq(indexNum).attr("checked", true);
            });
        });
    };
    return $(this).each(function () {
    ...
    var singleSelect = function (parentObj) {
        ...
        titObj.click(function (e) {
            ...
            if (itemObj.is(":hidden")) {
                ...
                //位于其它无素的上面
                arrowObj.css("z-index", "9999");
                itemObj.css("z-index", "9999");
                //显示下拉框
                arrowObj.show();
                itemObj.show();
            } else {
