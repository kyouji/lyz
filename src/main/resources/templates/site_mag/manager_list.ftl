...
<form name="form1" method="post" action="/Verwalter/manager/list" id="form1">
<div>
...
<div class="toolbar-wrap">
  <div id="floatHead" class="toolbar" style="position: static; top: 42px;">
    <div class="l-list">
      <ul class="icon-list">
        <li><a class="add" href="/Verwalter/manager/edit"><i></i><span>新增</span></a></li>
    ...
    <#if manager_page??>
        <#list manager_page.content as item>
            <tr>
                ...
                <td align="center"><a href="/Verwalter/manager/edit?id=${item.id?c}">${item.username!""}</a></td>
                <td align="center">${item.realName!""}</td>
                <td align="center">${item.telephone!""}</td>
                <td align="center">${item.email!""}</td>
                <td align="center">${item.createTime!""}</td>
                <td align="center"><#if item.isEnable?? && item.isEnable><font color="#009900">正常</font><#else>禁用</#if></td>
                <td align="center">
                    <input name="listSortId" type="text" value="${item.sortId!""}" class="sort" onkeydown="return checkNumber(event);">
                </td>
                <td align="center">
                    <a href="/Verwalter/manager/edit?id=${item.id?c}">修改</a>
                </td>
            </tr>
        </#list>
    </#if>
